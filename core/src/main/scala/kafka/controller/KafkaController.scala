...
class KafkaController(val config: KafkaConfig, zkClient: KafkaZkClient, time: Time, metrics: Metrics, initialBrokerInfo: BrokerInfo,
  ...
  private def onControllerFailover() {
    ...
    // The notification event (ControllerEvent) for each Handler will eventually call the controllerevent.process() method inside the ControllerEventManager
    // 注册和调用路径：brokerChangeHandler=>BrokerChange,topicChangeHandler=>TopicChange
    val childChangeHandlers = Seq(brokerChangeHandler, topicChangeHandler, topicDeletionHandler, logDirEventNotificationHandler,
      isrChangeNotificationHandler)
    // 把各种事件都一股脑注册到zk的子节点变化通知中
    childChangeHandlers.foreach(zkClient.registerZNodeChildChangeHandler)
    //注册PreferredReplicaLeaderElection和PartitionReassignment事件，事件回调方法process()
    val nodeChangeHandlers = Seq(preferredReplicaElectionHandler, partitionReassignmentHandler)
    nodeChangeHandlers.foreach(zkClient.registerZNodeChangeHandlerAndCheckExistence)

    info("Deleting log dir event notifications")
    // 撤销ZK上的注册的监视器: /log_dir_event_notification/{序号}
    zkClient.deleteLogDirEventNotifications()
    info("Deleting isr change notifications")
    // 撤销ZK上的注册的监视器:  /isr_change_notification/isr_change_{序号}
